 ...
 // const {institutionUser, isLoading, isError, isSuccess, message} = useSelector((state) => state.auth)
 
 const { institutionUserData, isLoading, isError, isSuccess, message } = useSelector((state) => state.institution);
 
 useEffect(() => {
     if (isError) {
         toast.error(message);
         console.log("page not opened");
     }
 
     if (isSuccess || institutionUser.institution.isVerified) {
         console.log(institutionUserData);
         navigate(`/institution/${institutionUserData._id}/dashboard`);
     }
 
     dispatch(reset());
 }, [institutionUserData, isError, isSuccess, message, dispatch]);
   ...
   //     if(isSuccess || user.institution.isVerified) {
   //         console.log(user)
   //         navigate(`/institution/${institutionUser.institution._id}/dashboard`)
   //     }
 ...
 return (
   <div className='flex flex-col md:w-1/2 m-auto flex-1 items-center gap-y-[50px] p-5'>
       ...
       <div className='flex flex-col'>
           ...
           <p className='text-[#6B3FA0] text-center'>{user.institution.emailAddress}</p>
       </div>
